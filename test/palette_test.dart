...
main() {
  ...
  test("Test palette generation", () async {
    ...
    canvas.drawRect(new Rect.fromLTRB(0.0, 0.0, 1000.0, 500.0),
        new Paint()..color = new Color(0xFFFF0000));
    canvas.drawRect(new Rect.fromLTRB(0.0, 501.0, 1000.0, 1000.0),
        new Paint()..color = new Color(0xFF00FF00));
    // paint more blue to make our dominant swatch blue, and test below.
    canvas.drawRect(new Rect.fromLTRB(0.0, 1001.0, 1000.0, 1600.0),
        new Paint()..color = new Color(0xFF0000FF));
    ...
    // should be blue
    expect(dominant.rgb.blue, greaterThan(247));
  });
  ...
  test('Output colors to disk', () async {
    ...
    var bytes = new Io.File(INPUT_IMAGE).readAsBytesSync();
    ...
    output.writeAsBytesSync(png.buffer.asUint8List());
      ...
      skip:
          "This is for visually debugging generated Swatches of a Palette from an image.");
}
